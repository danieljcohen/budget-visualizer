    ...
    <div className="space-y-8">
      ...
      <div className="bg-gradient-to-br from-navy-800/80 to-navy-900/80 backdrop-blur-sm rounded-3xl p-8 border border-navy-700/50 shadow-2xl">
        <div className="text-center mb-8">
          ...
          <p className="text-navy-300">
            How the total budget is distributed across committees
          </p>
        </div>
        ...
        <div className="h-[400px] relative">
          <ResponsivePie
            ...
            transitionMode="pushIn"
            tooltip={({ datum }) => {
              const itemData = data.allocationBreakdown.find(d => d.id === datum.id);
              return (
                <div className="bg-navy-800 px-4 py-3 rounded-lg shadow-xl border border-navy-600">
                  ...
                  <p className="text-white">
                    Allocated: <span className="text-gold-400 font-bold">{formatCurrency(datum.value)}</span>
                  </p>
                  <p className="text-white">
                    Spent: <span className="text-pink-400 font-bold">{formatCurrency(itemData?.spent || 0)}</span>
                  </p>
                  <p className="text-navy-400 text-sm mt-1">
                    {((datum.value / data.totalBudget) * 100).toFixed(1)}% of total budget
                  </p>
                  <div className="mt-2 pt-2 border-t border-navy-600">
                    <p className="text-sm">
                      Utilization: <span className={`font-bold ${(itemData?.utilization || 0) > 75 ? 'text-red-400' : 'text-emerald-400'}`}>
                        {(itemData?.utilization || 0).toFixed(1)}%
                      </span>
                    </p>
                  </div>
                </div>
          ...
          <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
            <div className="text-center">
              <p className="text-3xl font-bold text-white">Allocated</p>
              <p className="text-navy-400 text-sm">by chair</p>
            </div>
          </div>
        </div>
        ...
        <div className="mt-8 grid gap-3">
          {data.allocationBreakdown.map((item) => (
            <div 
              key={item.id}
              className="flex items-center justify-between p-4 rounded-xl bg-navy-800/30 border border-navy-700/20"
            >
              ...
              <div className="flex items-center gap-6">
                <div className="text-right">
                  ...
                  <p className="text-navy-400 text-xs">
                    {((item.value / data.totalBudget) * 100).toFixed(1)}% allocated
                  </p>
                </div>
                <div className="w-24">
                  <div className="flex justify-between text-xs mb-1">
                    <span className="text-navy-400">Used</span>
                    <span className={`font-medium ${item.utilization > 75 ? 'text-red-400' : 'text-emerald-400'}`}>
                  ...
                  <div className="h-2 bg-navy-700 rounded-full overflow-hidden">
                    <div 
                      ...
                      style={{ 
                        ...
                        backgroundColor: item.utilization > 75 ? '#f87171' : '#34d399'
                      }}
                    />
                  </div>
                </div>
